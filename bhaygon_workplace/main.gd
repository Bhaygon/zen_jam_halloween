...
func start_game():
	...
	add_child(r)
	var reset_time = 1.5
	$Player.reset(Vector2($CameraFollow.position.x + 10, 0), reset_time)
	timer = 0.0
	score = 0
	$HUD.hide_play()
	$HUD.show_message(str(3))
	await get_tree().create_timer(reset_time / 3).timeout
...
func _on_player_item_pick(value: int):
	score += value
